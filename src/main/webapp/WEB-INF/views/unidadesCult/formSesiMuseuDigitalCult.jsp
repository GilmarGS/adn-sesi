...
<script type="text/javascript">
	function validar(){
		if(sesimuseudigital.metaEventosCulturais.value == ""){
			document.querySelector('input[name="metaEventosCulturais"]').value = 0;
		}
		if(sesimuseudigital.metaCulturaEventos.value == ""){
			document.querySelector('input[name="metaCulturaEventos"]').value = 0;
		}
		if(sesimuseudigital.realizacaoEventosCulturais.value == ""){
			document.querySelector('input[name="realizacaoEventosCulturais"]').value = 0;
		}
		if(sesimuseudigital.realizacaoCulturaEventos.value == ""){
	...
	<div class="container">	
				...
				<div class="collapse navbar-collapse" id="navbarNavDropdown">
					<ul class="navbar-nav">
						...
						<li class="nav-item"><a class="nav-link" href="/unidadescult/formDma">DMA</a></li>
						<li class="nav-item active"><a class="nav-link disable" href="#">SESI Museu Digital</a></li>
					</ul>
				</div>
			</nav>
		<h3>ANÁLISE DE DESEMPENHO <%= anoAtual %> - SESI MUSEU DIGITAL</h3><br>
	</div>
	<form name="sesimuseudigital" class="form-siignin" action="/sesimuseudigitalCult" method="post">
		<div class="container">		  
			...
			<label class="text-primary">CULTURA</label>	<br>					
						<label>EVENTOS CULTURAIS</label>
						  <div class="form-row">
    						<div class="col">
      							<input type="text" name="metaEventosCulturais" class="form-control" placeholder="REALIZADO" autocomplete="off">
							</div>
					  	</div>			
  					</div>
  				</div>
